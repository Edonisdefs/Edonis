 ...
 * The camera is never animated directly — everything lands here so the three
 * inputs can never fight each other.
 */

import { MathUtils, Vector3 } from 'three';
import { SHOTS } from './state.js';
...
/**
 * The shots are framed for 16:9. Narrower containers widen the lens to hold the
...
/** 0 on a landscape container, 1 on a phone held upright. */
...
export function updateCamera(camera, cam, delta) {
  const dt = Math.min(delta, 0.05);

  // Frame-rate independent smoothing.
  cam.mouse.lerp(cam.mouseTarget, 1 - Math.pow(0.0015, dt));
  ...
  // Portrait containers pull the shot back so nothing important is cropped, and
  // aim higher so the building sits low in the tall frame rather than floating
  // above an empty foreground.
  const portrait = portraitAmount(camera.aspect);
  if (portrait > 0.001) {
    if (cam.dolly !== 1) {
      const dolly = 1 + (cam.dolly - 1) * portrait;
  ...
  if (amp > 0.001) {
    ...
    _pos.addScaledVector(_right, -cam.mouse.x * Math.tan(PARALLAX_X) * dist * amp);
    _pos.addScaledVector(_up, cam.mouse.y * Math.tan(PARALLAX_Y) * dist * amp);
  }
